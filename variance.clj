...
(def towns ["Rome", "London", "Paris", "NY", "Vancouver", "Sydney", "Bangkok", "Tokyo",
         "Beijing", "Lima", "Montevideo", "Caracas", "Madrid", "Berlin", "Moscow"])


(require '[clojure.string :as str])

(defn average-from-list
  [numbers]
  (/ (apply + numbers) (count numbers)))

(defn get-town-data-into-list
  [twn strng]
  (let [town-data (get (into {} (map #(str/split % #"\:") (str/split strng #"\n"))) twn)]
    (if (nil? town-data)
      nil
      (map #(read-string %) (vals (into {} (map #(str/split % #"\ ") (str/split town-data  #"\,"))))))))

(defn mean [twn strng]
  (let [town-data (get-town-data-into-list twn strng)]
    (if (nil? town-data)
      -1
      (average-from-list town-data))))

(defn compute-variance
  [numbers average]
  (average-from-list (map #(Math/pow (- average %) 2) numbers)))

(defn variance [twn strng]
  (let [town-data (get-town-data-into-list twn strng)]
    (if (nil? town-data)
      -1
      (compute-variance town-data (average-from-list town-data)))))
